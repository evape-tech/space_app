...
const RechargeItem = ({ recharge }) => {
  ...
  const { updatedAt, points, balance } = recharge;
  return (
    <div className="flex  p-[15px] border-b-2 justify-between items-center w-full bg-white">
      <div className="text-[#828282] text-[15px]">
        <div>{dayjs(updatedAt).format("YYYY/MM/DD")}</div>
        <div>{dayjs(updatedAt).format("HH:mm:ss")}</div>
      </div>
      <div>
        <div className="text-[17px] font-medium">充值 {points}</div>
        <p className="text-[15px] text-[#BDBDBD] ">目前餘額 {balance}</p>
      </div>
    </div>
  );
};
